...
| REC-01 | Win/Linux/macOS | P1, P2 | Record fixture for 3–5 seconds; let automatic endpointing finish it; inspect status and the recovery-recording directory. | Capture begins after stream start, automatic endpointing preserves configured post-roll, prepares mono 16 kHz audio in memory, starts transcription, and creates no normal capture WAV. Capture exact latency phases. | **NOT VERIFIED** |
...
| REC-03 | Win/Linux/macOS | P1, P2 | Let recording run to configured maximum duration. | Recording stops deterministically and retains the graceful configured post-roll, trimming, and terminal-preview path; it remains distinct from explicit stop's low-latency retirement path. | **NOT VERIFIED** |
...
| REC-07 | Win/Linux/macOS | P1, P2 | Enable Rolling preview, dictate until a partial is visible, then explicitly stop while preview decoding is busy. Repeat with a deliberately slow preview diagnostic build if available. Inspect the debug latency summary and immediately attempt a new recording before preview retirement completes. | No terminal preview revision appears after explicit stop and late preview text never replaces final state. Exactly one final request is registered without waiting for physical preview join, but its result is not applied or pasted until retirement completes. New microphone/session use remains blocked until the retiring worker joins. The latency summary separately reports stop to capture-result availability, stop to final-audio-ready, final-audio-ready to final-request registration, preview retirement requested/completed, and final-request registration to final text; unobserved phases remain absent. If retirement misses its bounded acknowledgement deadline, final output is cancelled once and admission stays blocked until the handle is reaped. | **NOT VERIFIED** |
