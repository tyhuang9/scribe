...
| Auto qualification policy | Stage 5 embeds a strict canonical Windows x64 policy, rejects unknown or mismatched pack/model/provider/device/driver/evidence facts, and filters the checked-in zero-entry policy before any provider probe. Auto reserves CPU inside the four-attempt bound; explicit GPU remains CPU-strict. Five-cold/twenty-warm hardware evidence, device-loss, suspend/resume, driver-update, and packaged installer lanes were not run. | **PASS automated gate / hardware NOT VERIFIED** |
