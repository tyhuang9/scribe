...
| Area | Evidence | Status |
| --- | --- | --- |
| Repository gates | `cargo fmt --all -- --check`; `cargo check --all-targets --all-features`; `cargo clippy --all-targets --all-features -- -D warnings`; `cargo test --all-targets --all-features -- --test-threads=1` | **PASS** - 909 tests discovered, 898 passed, 0 failed, 11 ignored because they require explicit local runtime/fixture environments |
...
| Display and control separation | Distinct pass-through/non-activating hardening profiles, transparency-bit verification, 320 x 52 Compact status and 440 x 140 Live preview geometry, reserved control bounds, and 44 x 44 AccessKit button tests | **PASS automated contracts** - native HWND hit-testing, activation, taskbar/Alt+Tab, and mixed-DPI alignment remain NOT VERIFIED |
