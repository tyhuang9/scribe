...
### Stage 4/5 Windows GPU worker-pack checkpoint

Initial checkpoint recorded 2026-08-29 on Windows x64; later fixture updates are
dated explicitly below. This is automated fixture/hardware evidence
...
| Fixture-only CPU vs explicit-Vulkan evidence harness | `scripts/run-windows-vulkan-evidence.ps1` builds a fresh isolated CPU worker and fixture-signed Vulkan pack from one clean revision, runs the exact ignored test serially, captures exactly five cold and 20 warm runs per backend, and emits a new bounded metadata-only untrusted artifact plus its independently captured SHA-256 only on stdout. The on-disk path remains untrusted: consumers must separately retain that stdout digest and pass it to `Read-ScribeVerifiedEvidenceReport`, which reads, hashes, compares, and strictly schema-validates one bound in-memory representation without reopening the path; comments, trailing commas, duplicate names, coerced token types, and noncanonical unsigned-integer lexemes fail closed. A PowerShell session that already loaded an older native evidence type must be restarted and is rejected at preflight load. It preserves the byte-identical canonical empty Windows Auto manifest and rejects production-signing inputs, reparse/activation paths, mismatched fixture hashes, and a busy NVIDIA device when `nvidia-smi` is available. | **PASS fixture capture, 2026-09-13** — matching release CPU parent/worker and Vulkan pack at clean `4920b6174de368f44c36cce146ecd5e23e0ab70f`, RTX 4080 SUPER, 50 measured requests with transcript parity. Report SHA-256 `cc1b45487832f5c9e55fbf34555efaa4c6696e54be8761ff6dd2dd0215b20516`; [bounded evidence](https://github.com/tyhuang9/scribe/pull/161). Not full-app latency, production qualification or Auto eligibility. |
| Fixture-only CPU vs explicit-CUDA evidence harness | `scripts/run-windows-cuda-evidence.ps1` clean-builds a release CPU worker and fixture-signed CUDA pack from the same revision, requires one stable CUDA/NVIDIA/discrete PCI binding and an idle NVIDIA baseline, explicitly waits for the exact test process and its exit code, captures exactly five cold and 20 warm runs per backend, and emits a distinct metadata-only report bound to CPU worker, pack, model, WAV, and source digests. Capture the printed digest separately and consume only through `Read-ScribeVerifiedCudaEvidenceReport`. | **PASS canonical fixture capture, 2026-09-14** — fresh matching artifacts at clean `92c93b66dd79bdb73fe380cfb2deaea68375ce93`, RTX 4080 SUPER, 50 measured requests with parity; report SHA-256 `26262aaf6cd50ada4874864304df5707fe463dc982ff504d7bb36843ef28fee0`. CPU cold/warm p95 968/355 ms; CUDA 1781/128 ms. The original `31312d4` publication-race failure and its separate operator recovery remain distinct [historical records](GPU_WORKER_PACKS.md#cuda-timing-and-waited-runner-checkpoint-2026-09-13). Cold CUDA fails the 10% criterion; no production or Auto qualification is established. |
| CUDA startup phase attribution | Schema v2 records disjoint per-generation resolution/verification, executable revalidation, spawn/supervision, and validated parent Hello intervals. Cold records must fit their matching request; warm startup is null after priming. Independently recompute percentiles and sum phases per request. | **PASS operator recovery, 2026-09-23** — clean `26df7731cd9c8e74758fe1e621cc4b4d73fad19c`, same device/driver, 50 measured requests with parity; report SHA-256 `a723f6bea3f3fe3c9a1bc690b2bcd3ab0a9cfffe3a9cc90cf0f691f92ea93ff1`. CPU cold/warm p95 1016/373 ms; CUDA 1701/143 ms. CUDA resolution p50/p95 800/808 ms directs investigation, not removal of verification. The original canonical attempt failed idle-GPU admission before any benchmark; recovery reused exact verified artifacts after the GPU became idle. [Capture identities and limits](GPU_WORKER_PACKS.md#cuda-startup-attribution-checkpoint-2026-09-23). No full-app, production trust, installer, promotion, or Auto qualification is established. |
| CUDA provider | Toolkit/nvcc was absent at the initial checkpoint. On 2026-09-13, isolated CUDA Toolkit 12.8.1 / nvcc 12.8.93 built a fixture release pack at clean `4920b6174de368f44c36cce146ecd5e23e0ab70f`; the matching release-parent CUDA smoke passed on RTX 4080 SUPER, stable `native:0000:01:00.0`, driver `windows-display:32.0.16.1692`, with known phrase, warm reuse and zero fake CPU fallback launches. Smoke SHA-256 `02b4aaba5d7b0425ae6138aa1c34f6bec7ea341f9895e5cf59d216d422350f8d`; [artifact identities and limits](https://github.com/tyhuang9/scribe/pull/161). Production still requires reviewed exact SDK inventory, trust and signing inputs. | **PASS isolated fixture smoke / production fail closed** |
...
| Auto qualification policy | Stage 5 embeds a strict canonical Windows x64 policy, rejects unknown or mismatched pack/model/provider/device/driver/evidence facts, and filters the checked-in zero-entry policy before any provider probe. Auto reserves CPU inside the four-attempt bound; explicit GPU remains CPU-strict. The fixture results above do not satisfy representative release qualification, device-loss, suspend/resume, driver-update, power-policy or packaged-installer lanes. | **PASS automated gate / production hardware qualification NOT VERIFIED** |
...
| STT-08 | Win/Linux/macOS | P1, P2, P3 | Change Auto/GPU/CPU-only acceleration preference where supported; run fixture on each available mode. | Auto probes only release-qualified packs, rechecks the stable device/driver after Hello, excludes discrete GPUs on battery, follows CUDA → Vulkan → CPU on Windows NVIDIA and Vulkan → CPU on Windows AMD/Intel, and reports its evidence/power/health/fallback reasons. With the checked-in zero-entry policy it selects CPU without a GPU probe. Explicit CPU is honored; explicit GPU uses only a verified compatible pack/device and fails clearly without silent CPU fallback. | **NOT VERIFIED** |
...
| STT-11 | Windows x64 | Exact `parakeet-tdt-06b-v2-en-int8-onnx` private bundle, Sherpa 1.13.5, and its pinned fixture | Run the ignored real-bundle diagnostic against the exact private bundle and fixture. | **PASS — scoped gate only:** stage, child load/health/silence, normalized known-WAV equality, unload/reload, and activation passed in 258.29 seconds total. This is diagnostic elapsed time, not model latency. Cancellation, supervisor restart recovery, accelerator use, non-Windows behavior, latency, RAM, and other resource use remain **NOT VERIFIED**; no native-streaming, role, recommendation, or Supported claim follows. | **PASS — scoped fixture gate only** |
