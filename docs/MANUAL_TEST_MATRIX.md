...
| Fixture-only CPU vs explicit-CUDA evidence harness | `scripts/run-windows-cuda-evidence.ps1` clean-builds a release CPU worker and fixture-signed CUDA pack from the same revision, requires one stable CUDA/NVIDIA/discrete PCI binding and an idle NVIDIA baseline, explicitly waits for the exact test process and its exit code, captures exactly five cold and 20 warm runs per backend, and emits a distinct metadata-only report bound to CPU worker, pack, model, WAV, and source digests. Capture the printed digest separately and consume only through `Read-ScribeVerifiedCudaEvidenceReport`. | **PASS separate waited operator capture / canonical rerun UNVERIFIED** — matching artifacts at clean `31312d484f46039fca84ef802679213a48278d4c`, RTX 4080 SUPER, 50 measured requests and parity; report SHA-256 `6fcc13bd55954d6605ce548cfd687a40b3a75d1b69a839768e04cefd86f8af72`. CPU cold/warm p95 642/352 ms; CUDA 1718/123 ms. The original canonical attempt raced publication before its GUI test finished and remains failed; its pending report was not promoted. The wait fix passed contracts and actual GUI listing/nonzero-exit checks, but a fresh full canonical capture after the fix is unrun. No full-app latency, production trust, qualification, installer support, promotion, or Auto eligibility is established. |
