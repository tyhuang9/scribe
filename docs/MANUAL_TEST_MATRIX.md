...
| UI-08 | Windows | P1, P2, P4 | With Live preview, begin dictation from another app; verify taskbar, Alt+Tab, original target focus, body clicks, and the 44 x 44 X. Cancel once during Preparing and once during Recording. Repeat presentation in Compact status and Off. | The overlay has no taskbar/Alt+Tab entry and remains always on top without activation. Body clicks reach the underlying app; only the X accepts input and clicking it does not move keyboard focus. Live preview is a 600 x 62 glass pill showing a grapheme-safe tail in one visual line; Compact status is 320 x 52 and shows recording state/meter/time. Off stays hidden. If native control hardening fails, the X is absent while the passive display remains safe. | **NOT VERIFIED** |
...
| DL-06 | Windows x64 packaged build | Fresh disposable profile; disconnect network before launch | Build with `build-windows-release.ps1`, confirm the final bundle is outside Cargo's `target` tree, compare every staged file with `bundle-inventory.json`, and confirm the model and three attribution files are present. Disconnect the network, then launch, transcribe the local fixture, cancel one transcription, unload/reload, and restart. Repeat with the bundled model missing, truncated, and replaced by same-sized wrong bytes; choose Repair only after each failure. | The exact base.en model is immediately Installed/ready offline and cannot be removed. Packaging and startup require exact size/SHA-256. Missing/corrupt bytes fail closed with repair guidance and no automatic download; an explicit Repair uses the verified managed flow without deleting existing managed files. | **NOT VERIFIED** |
