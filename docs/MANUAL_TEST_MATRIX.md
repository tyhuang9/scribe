...
| Fixture-only CPU vs explicit-CUDA evidence harness | `scripts/run-windows-cuda-evidence.ps1` clean-builds a release CPU worker and fixture-signed CUDA pack from the same revision, requires one stable CUDA/NVIDIA/discrete PCI binding and an idle NVIDIA baseline, captures exactly five cold and 20 warm runs per backend, and emits a distinct metadata-only report bound to CPU worker, pack, model, WAV, and source digests. Capture the printed digest separately and consume only through `Read-ScribeVerifiedCudaEvidenceReport`. | **Physical capture NOT VERIFIED** — automated checks do not establish a physical run of this new performance harness. The separate CUDA smoke below is not timing evidence. No production trust, qualification, installer support, promotion, or Auto eligibility is established. |
