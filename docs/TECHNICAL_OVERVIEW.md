...
The release workflow’s GPU-pack input is only for non-publication validation.
...
`gpu_packs_required` forces both production-signed packs and fails closed
without matching reviewed trust and the external signing secret.
