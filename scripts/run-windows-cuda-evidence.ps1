...
function Get-ScribeEvidencePinnedMsvcEnvironment([string]$Builder, [string]$NativeArchive, [string]$UnusedOutput) {
    try {
        $output = @(& $Builder -Backend Cuda -PackVersion 'fixture-toolchain-check' -OutputDirectory $UnusedOutput -SigningMode Fixture -NativeArchiveDirectory $NativeArchive -ToolchainCheckOnly -ExportPinnedMsvcEnvironment)
    }
    catch {
...
if (-not $IsWindows) { throw 'Windows CUDA evidence capture is Windows x64 only.' }
if ([Environment]::Is64BitOperatingSystem -ne $true) { throw 'Windows CUDA evidence capture requires x64 Windows.' }
...
$cudaToolkit = Resolve-ScribeEvidencePath $CudaToolkitDirectory
Assert-ScribeEvidenceNoReparse $cudaToolkit
...
$finalEvidenceLeaf = 'windows-cuda-fixture-evidence.json'
$pendingEvidenceLeaf = "windows-cuda-fixture-evidence.pending-$([guid]::NewGuid().ToString('N')).json"
...
function Assert-ScribeCudaEvidenceCleanSource {
    $status=@(& $git -C $repositoryRoot status --porcelain=v1 --untracked-files=all)
    if($LASTEXITCODE-ne0-or$status.Count-ne0){throw 'CUDA evidence requires a clean source tree including untracked files.'}
}
Assert-ScribeCudaEvidenceCleanSource
...
$pinnedMsvcEnvironment = $null
$evidenceEnvironmentNames = @(
    'SCRIBE_CUDA_EVIDENCE_PACK_ROOT',
    'SCRIBE_CUDA_EVIDENCE_CPU_WORKER',
    'SCRIBE_CUDA_EVIDENCE_MODEL',
    'SCRIBE_CUDA_EVIDENCE_MODEL_SHA256',
    'SCRIBE_CUDA_EVIDENCE_WAV',
    'SCRIBE_CUDA_EVIDENCE_WAV_SHA256',
    'SCRIBE_CUDA_EVIDENCE_EXPECTED_PHRASE',
    'SCRIBE_CUDA_EVIDENCE_EXPECTED_STABLE_DEVICE',
    'SCRIBE_CUDA_EVIDENCE_OUTPUT',
    'SCRIBE_CUDA_EVIDENCE_NVIDIA_BASELINE_JSON'
)
...
try {
    ...
    $packRoot = Join-Path $workRoot 'fixture-cuda-pack'
    $packVersion = New-ScribeEvidenceFixturePackVersion $revision ([guid]::NewGuid().ToString('N').Substring(0, 12))
    & $packBuilder -Backend Cuda -PackVersion $packVersion -OutputDirectory $packRoot -SigningMode Fixture -NativeArchiveDirectory $nativeArchive -CargoTargetDirectory (New-ScribeEvidenceShortCargoTarget 'cuda')
    if ($LASTEXITCODE -ne 0) { throw 'Fresh fixture-signed CUDA pack build failed.' }
    $packManifest = Get-Content -LiteralPath (Join-Path $packRoot 'pack-manifest.json') -Raw | ConvertFrom-Json
    $packManifestDigest = (Get-FileHash -LiteralPath (Join-Path $packRoot 'pack-manifest.json') -Algorithm SHA256).Hash.ToLowerInvariant()
    ...
    if((& $git -C $repositoryRoot rev-parse --verify HEAD).Trim()-cne$revision){throw 'Source HEAD changed before CUDA precompile.'};Assert-ScribeCudaEvidenceCleanSource
    ...
    Invoke-ScribeEvidenceWithPinnedMsvcEnvironment $pinnedMsvcEnvironment {
        ...
        if ($observedCpuWorkerDigest -cne $cpuWorkerDigest) { throw 'Materialized CPU worker changed before CUDA evidence test precompilation.' }
        if ($null -ne $env:SCRIBE_BUILDING_WORKER) { throw 'CUDA evidence test precompilation must not use worker-build mode.' }
        $env:SCRIBE_BUNDLED_WORKER_SHA256 = $cpuWorkerDigest
        Invoke-ScribeEvidenceCargoWithCmakeRetry @('test', '--locked', '--offline', '--release', '--features', 'inference-worker', 'onnx_worker::tests::windows_cuda_fixture_evidence_captures_five_cold_and_twenty_warm_runs', '--no-run') 'CUDA evidence test precompilation failed.' $env:CARGO_TARGET_DIR $harnessBuildEnvironment
    }
    $testCandidates = @(Get-ChildItem -LiteralPath (Join-Path $env:CARGO_TARGET_DIR 'release\deps') -File -Filter 'local_transcriber-*.exe' | Where-Object { $_.Name -cmatch '^local_transcriber-[0-9a-f]{16}\.exe$' })
    if ($testCandidates.Count -ne 1) { throw 'CUDA evidence no-run phase did not produce exactly one direct-child test executable.' }
    $testExecutable = Assert-ScribeEvidenceSingleLinkFile $testCandidates[0].FullName 'CUDA evidence test executable' (512MB) $trustedFsutil
    $baseline = Get-ScribeVulkanEvidenceNvidiaBaseline $ExpectedStableDevice $trustedNvidiaSmi
    $env:SCRIBE_CUDA_EVIDENCE_PACK_ROOT = $packRoot
    $env:SCRIBE_CUDA_EVIDENCE_CPU_WORKER = $cpuWorker
    $env:SCRIBE_CUDA_EVIDENCE_MODEL = $model
    $env:SCRIBE_CUDA_EVIDENCE_MODEL_SHA256 = $ModelSha256
    $env:SCRIBE_CUDA_EVIDENCE_WAV = $wav
    $env:SCRIBE_CUDA_EVIDENCE_WAV_SHA256 = $WavSha256
    $env:SCRIBE_CUDA_EVIDENCE_EXPECTED_PHRASE = $ExpectedPhrase
    $env:SCRIBE_CUDA_EVIDENCE_EXPECTED_STABLE_DEVICE = $ExpectedStableDevice
    $env:SCRIBE_CUDA_EVIDENCE_OUTPUT = $pendingEvidenceOutput
    $env:SCRIBE_CUDA_EVIDENCE_NVIDIA_BASELINE_JSON = $baseline | ConvertTo-Json -Compress
    Invoke-ScribeEvidenceWithPinnedMsvcEnvironment $pinnedMsvcEnvironment {
        Set-ScribeEvidenceWorkerBuildMode $false
        $observedCpuWorkerDigest = (Get-FileHash -LiteralPath $cpuWorker -Algorithm SHA256).Hash.ToLowerInvariant()
        if ($observedCpuWorkerDigest -cne $cpuWorkerDigest) { throw 'Materialized CPU worker changed before exact CUDA evidence execution.' }
        if ((Get-FileHash -LiteralPath $model -Algorithm SHA256).Hash.ToLowerInvariant() -cne $ModelSha256 -or
            (Get-FileHash -LiteralPath $wav -Algorithm SHA256).Hash.ToLowerInvariant() -cne $WavSha256 -or
            (Get-FileHash -LiteralPath (Join-Path $packRoot 'pack-manifest.json') -Algorithm SHA256).Hash.ToLowerInvariant() -cne $packManifestDigest -or
            (& $git -C $repositoryRoot rev-parse --verify HEAD).Trim() -cne $revision) {
            throw 'CUDA evidence source, pack, model, or WAV changed before exact execution.'
        }
        Assert-ScribeCudaEvidenceCleanSource
        if ($null -ne $env:SCRIBE_BUILDING_WORKER) { throw 'Exact CUDA evidence execution must not use worker-build mode.' }
        $env:SCRIBE_BUNDLED_WORKER_SHA256 = $cpuWorkerDigest
        Invoke-ScribeEvidence $testExecutable @('onnx_worker::tests::windows_cuda_fixture_evidence_captures_five_cold_and_twenty_warm_runs', '--ignored', '--exact', '--test-threads=1') 'The exact CUDA evidence test failed.'
    }
}
catch {
...
if($null-eq$primaryFailure-and$secondaryFailures.Count-eq0){try{if((& $git -C $repositoryRoot rev-parse --verify HEAD).Trim()-cne$revision){throw 'Source HEAD changed before CUDA publication.'};Assert-ScribeCudaEvidenceCleanSource}catch{$primaryFailure=$_.Exception}}
$published = Complete-ScribeCudaEvidencePendingReport `
...
Write-Output "Fixture-only untrusted CUDA evidence path: $($published.Path)"
Write-Output "Independent consumer-bound SHA-256 (capture this stdout value): $($published.Digest)"
Write-Output 'The on-disk evidence path is untrusted without that independently captured digest; consume it only through Read-ScribeVerifiedCudaEvidenceReport.'
