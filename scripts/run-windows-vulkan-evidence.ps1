...
if ($ExpectedStableDevice -and $ExpectedStableDevice -cnotmatch '^native:[0-9a-f]{4}:[0-9a-f]{2}:[0-9a-f]{2}\.[0-7]$') { throw 'ExpectedStableDevice must be canonical lowercase native PCI identity.' }
...
try {
    ...
    $env:CARGO_TARGET_DIR = Join-Path $workRoot 'cpu-target'
    ...
    $cpuWorker = Assert-ScribeEvidenceFile (Join-Path $env:CARGO_TARGET_DIR 'release\scribe-inference-worker.exe') 'CPU worker' (512MB)
    ...
    $env:SCRIBE_VULKAN_EVIDENCE_NVIDIA_BASELINE_JSON = if ($null -eq $baseline) { $null } else { $baseline | ConvertTo-Json -Compress }
    $env:CARGO_TARGET_DIR = Join-Path $workRoot 'harness-target'
