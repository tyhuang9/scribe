...
function Assert-ScribeCudaWorkerStartupTimings($Set, [string]$Phase) {
    $startup = $Set.GetProperty('worker_startup_ms')
    if ($Phase -ceq 'warm') {
        if ($startup.ValueKind -ne [Text.Json.JsonValueKind]::Null) {
            throw 'CUDA warm worker startup must be null after unmeasured priming.'
        }
        return
    }
    if ($startup.GetArrayLength() -ne 5) {
        throw 'CUDA cold worker startup cardinality mismatch.'
    }
    $fields = @('resolve_ms', 'executable_revalidation_ms', 'spawn_ms', 'hello_ms')
    $index = 0
    foreach ($record in $startup.EnumerateArray()) {
        Assert-ScribeCudaJsonProperties $record $fields 'CUDA worker startup'
        # Subtract from the matching request budget to avoid overflow or floating-point rounding.
        [uint64]$remaining = $Set.GetProperty('end_to_end_ms')[$index].GetUInt64()
        foreach ($field in $fields) {
            $value = $record.GetProperty($field)
            Assert-ScribeCudaCanonicalUInt $value "CUDA worker startup $field"
            [uint64]$duration = $value.GetUInt64()
            if ($duration -gt $remaining) {
                throw 'CUDA worker startup phases exceed their matching end-to-end request.'
            }
