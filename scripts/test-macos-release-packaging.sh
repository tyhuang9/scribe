...
if grep -En 'strings .* \| grep -[^ ]*q' "$repo_root/scripts"/{build-macos-release.sh,verify-macos-release-package.sh}; then echo 'binary authority checks must consume the complete strings stream under pipefail.' >&2; exit 1; fi
