...
rg -F 'SCRIBE_MACOS_GPU_RELEASE_SECURITY_EPOCH must be a canonical u64 decimal.' "$repo_root/scripts/build-macos-release.sh" >/dev/null || { echo 'malformed release epoch rejection is missing.' >&2; exit 1; }
rg -F 'security epoch must be a positive canonical u64 decimal.' "$repo_root/scripts/build-macos-metal-worker-pack.sh" >/dev/null || { echo 'malformed pack epoch rejection is missing.' >&2; exit 1; }
...
rg -F 'signed target does not expose exactly the selected Keychain access group' "$repo_root/scripts/build-macos-release.sh" >/dev/null || { echo 'protected release must verify final effective desktop entitlements.' >&2; exit 1; }
